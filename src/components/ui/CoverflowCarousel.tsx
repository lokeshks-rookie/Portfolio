...
export function CoverflowCarousel({
  ...
  React.useEffect(() => {
    ...
    // Cards per millisecond (1 card every 3 seconds)
    const driftSpeed = 1 / 2000;

    const tick = (time: number) => {
      const dt = time - lastTime;
      lastTime = time;

      // Drift when not dragging and not settling from a flick
      if (!dragRef.current && rafRef.current === null) {
        posRef.current += driftSpeed * dt;
        // Keep target in sync so nudges/clicks don't jump back
        targetRef.current = posRef.current;

        const index = indexAt(posRef.current);
        // We use functional setState to avoid depending on 'selected' in the effect
        setSelected((prev) => (prev !== index ? index : prev));
        paint();
      }
          ...
          >
            {slides.map((slide, index) => (
              ...
              >
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center p-8 pb-12 bg-[var(--color-ink-2)] border border-white/10">
                  {slide.icon ? (
                    <div className="w-1/2 h-1/2 flex items-center justify-center text-[var(--color-base)] opacity-90 drop-shadow-md">
                      {slide.icon}
                    </div>
                  ) : (
                    slide.src && <img
                ...
                <div className="absolute bottom-2 inset-x-0 flex justify-center">
                  <span className="text-[var(--color-base)] text-sm font-semibold tracking-widest uppercase">{slide.title}</span>
                </div>
              </div>
            ))}
          </div>
